...
class elegantwp_posts_widget_eight extends WP_Widget
{

  function __construct()
  {
    $widget_ops = array( 'classname' => 'elegantwp-featured-posts-widget-eight', 'description' => esc_html__( 'This widget displays recent/popular/random posts or displays posts from a given category or tag.', 'elegantwp-pro' ) );
    $control_ops = array( 'id_base' => 'elegantwp-featured-posts-widget-eight-id' );
    parent::__construct( 'elegantwp-featured-posts-widget-eight-id', esc_html__( 'ElegantWP Featured Posts - Style 8', 'elegantwp-pro' ), $widget_ops, $control_ops );
  }
  ...
  function form($instance)
  {
    ...
    $defaults = array( 'title' => esc_html__( 'Featured Posts - Style 8', 'elegantwp-pro' ), 'number' => 1, 'show_thumbnail' => true, 'show_cats' => true, 'show_author' => true, 'show_date' => true, 'show_comments' => true, 'post_type' => 'recentposts', 'featured_cat' => '', 'featured_tag' => '' );
...
<?php
  ...
  function widget($args,$instance)
  {
    ...
    if ($elegantwplabelposts->have_posts()) :
        ...
        <div class="elegantwp-fp08-posts">
        <?php while ($elegantwplabelposts->have_posts()) : $elegantwplabelposts->the_post();  ?>
        <div class="elegantwp-fp08-post">
            <?php if(has_post_thumbnail() && $show_thumbnail) { ?>
            <div class="elegantwp-fp08-post-thumbnail">
                <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_post_thumbnail('elegantwp-featured-image', array('class' => 'elegantwp-fp08-post-thumbnail-img', 'title' => get_the_title())); ?></a>
            </div>
            <?php } ?>
            <?php if(has_post_thumbnail() && $show_thumbnail) { ?><div class="elegantwp-fp08-post-details"><?php } ?>
            <?php if(!has_post_thumbnail() || !$show_thumbnail) { ?><div class="elegantwp-fp08-post-details-full"><?php } ?>
            <?php
            if($show_cats) {
            ...
            ?>
            <h3 class="elegantwp-fp08-post-title"><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_title(); ?></a></h3>
            <div class="elegantwp-fp08-post-footer">
            <?php if($show_author) { ?><span class="elegantwp-fp08-post-author elegantwp-fp08-post-meta"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>"><?php echo esc_html( get_the_author() ); ?></a></span><?php } ?>
            <?php if($show_date) { ?><span class="elegantwp-fp08-post-date elegantwp-fp08-post-meta"><?php echo esc_html( get_the_date('d-m-Y') ); ?></span><?php } ?>
            <?php if($show_comments) { ?><span class="elegantwp-fp08-post-comment elegantwp-fp08-post-meta"><?php comments_popup_link( esc_attr__( '0 Comments', 'elegantwp-pro' ), esc_attr__( '1 Comment', 'elegantwp-pro' ), esc_attr__( '% Comments', 'elegantwp-pro' ) ); ?></span><?php } ?>
            </div>
...
function elegantwp_posts_widget_eight_function() {
    register_widget( 'elegantwp_posts_widget_eight' );
}
add_action( 'widgets_init', 'elegantwp_posts_widget_eight_function' );