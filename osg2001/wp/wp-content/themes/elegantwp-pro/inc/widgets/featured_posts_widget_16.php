...
class elegantwp_posts_widget_sixteen extends WP_Widget
{

  function __construct()
  {
    $widget_ops = array( 'classname' => 'elegantwp-featured-posts-widget-sixteen', 'description' => esc_html__( 'This widget displays recent/popular/random posts or displays posts from a given category or tag.', 'elegantwp-pro' ) );
    $control_ops = array( 'id_base' => 'elegantwp-featured-posts-widget-sixteen-id' );
    parent::__construct( 'elegantwp-featured-posts-widget-sixteen-id', esc_html__( 'ElegantWP Featured Posts - Style 16', 'elegantwp-pro' ), $widget_ops, $control_ops );
  }
  ...
  function form($instance)
  {
    ...
    $defaults = array( 'title' => esc_html__( 'Featured Posts - Style 16', 'elegantwp-pro' ), 'number' => 5, 'show_thumbnail' => true, 'show_cats' => true, 'show_author' => true, 'show_date' => true, 'show_comments' => true, 'post_type' => 'recentposts', 'featured_cat' => '', 'featured_tag' => '' );
    ...
    if ($elegantwplabelposts->have_posts()) :
        ...
        <div class="elegantwp-fp16-post">
            ...
            <div class="elegantwp-fp16-post-thumbnail">
                <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_post_thumbnail('elegantwp-featured-image', array('class' => 'elegantwp-fp16-post-thumbnail-img', 'title' => get_the_title())); ?></a>
            </div>
            <?php } else { ?>
            <div class="elegantwp-fp16-post-thumbnail">
                <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><img src="<?php echo esc_url( get_template_directory_uri() . '/assets/images/no-image-2-1.jpg' ); ?>" class="elegantwp-fp16-post-thumbnail-img"/></a>
            </div>
            <?php } ?>
            <?php } else { ?>
            <div class="elegantwp-fp16-post-thumbnail">
            ...
            if ( 'post' == get_post_type() ) {
                ...
                if ( $categories_list ) {
                    ...
                    printf( '<div class="elegantwp-fp16-post-categories">' . __( '<span class="screen-reader-text">Posted in </span>%1$s', 'elegantwp-pro' ) . '</div>', $categories_list ); // WPCS: XSS OK.
                }
            }
            }
            ?>
            <h3 class="elegantwp-fp16-post-title"><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_title(); ?></a></h3>
            <div class="elegantwp-fp16-post-footer">
            <?php if($show_author) { ?><span class="elegantwp-fp16-post-author elegantwp-fp16-post-meta"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>"><?php echo esc_html( get_the_author() ); ?></a></span><?php } ?>
            <?php if($show_date) { ?><span class="elegantwp-fp16-post-date elegantwp-fp16-post-meta"><?php echo get_the_date('d-m-Y'); ?></span><?php } ?>
            <?php if($show_comments) { ?><span class="elegantwp-fp16-post-comment elegantwp-fp16-post-meta"><?php comments_popup_link( esc_attr__( '0 Comments', 'elegantwp-pro' ), esc_attr__( '1 Comment', 'elegantwp-pro' ), esc_attr__( '% Comments', 'elegantwp-pro' ) ); ?></span><?php } ?>
            </div>
            </div>
...
function elegantwp_posts_widget_sixteen_function() {
    register_widget( 'elegantwp_posts_widget_sixteen' );
}
add_action( 'widgets_init', 'elegantwp_posts_widget_sixteen_function' );