...
class elegantwp_posts_widget_nine extends WP_Widget
{

  function __construct()
  {
    $widget_ops = array( 'classname' => 'elegantwp-featured-posts-widget-nine', 'description' => esc_html__( 'This widget displays recent/popular/random posts or displays posts from a given category or tag.', 'elegantwp-pro' ) );
    $control_ops = array( 'id_base' => 'elegantwp-featured-posts-widget-nine-id' );
    parent::__construct( 'elegantwp-featured-posts-widget-nine-id', esc_html__( 'ElegantWP Featured Posts - Style 9', 'elegantwp-pro' ), $widget_ops, $control_ops );
  }
  ...
  function form($instance)
  {
    ...
    $defaults = array( 'title' => esc_html__( 'Featured Posts - Style 9', 'elegantwp-pro' ), 'number' => 1, 'show_thumbnail' => true, 'show_cats' => true, 'show_tags' => true, 'show_author' => true, 'show_date' => true, 'show_comments' => true, 'show_content' => true, 'show_sharebtns' => true, 'hide_rposts' => true, 'post_type' => 'recentposts', 'featured_cat' => '', 'featured_tag' => '' );
...
?>
        ...
        <p>
            <input class="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'show_tags' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'show_tags' ) ); ?>" type="checkbox" <?php checked( $instance['show_tags'] ); ?>  />
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_tags' ) ); ?>"><?php esc_html_e( 'Display Post Tags?','elegantwp-pro' ); ?></label>
        </p>
        <p>
        ...
        <p>
            <input class="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'show_content' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'show_content' ) ); ?>" type="checkbox" <?php checked( $instance['show_content'] ); ?>  />
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_content' ) ); ?>"><?php esc_html_e( 'Display Post Content?','elegantwp-pro' ); ?></label>
        </p>
        <p>
            <input class="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'show_sharebtns' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'show_sharebtns' ) ); ?>" type="checkbox" <?php checked( $instance['show_sharebtns'] ); ?>  />
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_sharebtns' ) ); ?>"><?php esc_html_e( 'Display Share Buttons?','elegantwp-pro' ); ?></label>
        </p>
        <p>
...
<?php
  ...
  function update($new_instance,$old_instance)
  {
    ...
    $instance['show_sharebtns'] = isset( $new_instance['show_sharebtns'] ) ? (bool) $new_instance['show_sharebtns'] : false;
  ...
  function widget($args,$instance)
  {
    ...
    $show_sharebtns = isset( $instance['show_sharebtns'] ) ? (bool) $instance['show_sharebtns'] : false;
    ...
    if ($elegantwplabelposts->have_posts()) :
        ...
        <div class="elegantwp-fp09-posts">
        <?php while ($elegantwplabelposts->have_posts()) : $elegantwplabelposts->the_post();  ?>
        <div class="elegantwp-fp09-post">
            <?php if(has_post_thumbnail() && $show_thumbnail) { ?>
            <div class="elegantwp-fp09-post-thumbnail">
                <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_post_thumbnail('elegantwp-featured-image', array('class' => 'elegantwp-fp09-post-thumbnail-img', 'title' => get_the_title())); ?></a>
            </div>
            <?php } ?>
            <?php if(has_post_thumbnail() && $show_thumbnail) { ?><div class="elegantwp-fp09-post-details"><?php } ?>
            <?php if(!has_post_thumbnail() || !$show_thumbnail) { ?><div class="elegantwp-fp09-post-details-full"><?php } ?>
            <?php
            if($show_cats) {
            ...
            ?>
            <h3 class="elegantwp-fp09-post-title"><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_title(); ?></a></h3>
            <div class="elegantwp-fp09-post-footer">
            <?php if($show_author) { ?><span class="elegantwp-fp09-post-author elegantwp-fp09-post-meta"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>"><?php echo esc_html( get_the_author() ); ?></a></span><?php } ?>
            <?php if($show_date) { ?><span class="elegantwp-fp09-post-date elegantwp-fp09-post-meta"><?php echo esc_html( get_the_date('d-m-Y') ); ?></span><?php } ?>
            <?php if($show_comments) { ?><span class="elegantwp-fp09-post-comment elegantwp-fp09-post-meta"><?php comments_popup_link( esc_attr__( '0 Comments', 'elegantwp-pro' ), esc_attr__( '1 Comment', 'elegantwp-pro' ), esc_attr__( '% Comments', 'elegantwp-pro' ) ); ?></span><?php } ?>
            </div>
            <?php if($show_content) { ?>
            ...
            <?php } ?>
            <?php if($show_sharebtns || $show_tags) { ?>
            <div class="elegantwp-fp09-post-footer">
            <?php if($show_sharebtns) { ?><?php echo wp_kses_post( force_balance_tags( elegantwp_social_sharing_buttons() ) ); ?><?php } ?>
            <?php
            if($show_tags) {
            if ( 'post' == get_post_type() ) {
...
function elegantwp_posts_widget_nine_function() {
    register_widget( 'elegantwp_posts_widget_nine' );
}
add_action( 'widgets_init', 'elegantwp_posts_widget_nine_function' );