...
class elegantwp_posts_widget_four extends WP_Widget
{

  function __construct()
  {
    $widget_ops = array( 'classname' => 'elegantwp-featured-posts-widget-four', 'description' => esc_html__( 'This widget displays recent/popular/random posts or displays posts from a given category or tag.', 'elegantwp-pro' ) );
    $control_ops = array( 'id_base' => 'elegantwp-featured-posts-widget-four-id' );
    parent::__construct( 'elegantwp-featured-posts-widget-four-id', esc_html__( 'ElegantWP Featured Posts - Style 4', 'elegantwp-pro' ), $widget_ops, $control_ops );
  }

  function form($instance)
  {
    $poststyles = array( 'elegantwp-fp04-post-style-1' => esc_html__( 'Post Style 1', 'elegantwp-pro' ), 'elegantwp-fp04-post-style-2' => esc_html__( 'Post Style 2', 'elegantwp-pro' ), 'elegantwp-fp04-post-style-3' => esc_html__( 'Post Style 3', 'elegantwp-pro' ) );
    ...
    $defaults = array( 'title' => esc_html__( 'Featured Posts - Style 4', 'elegantwp-pro' ), 'number' => 4, 'show_thumbnail' => true, 'show_cats' => true, 'show_author' => true, 'show_date' => true, 'show_comments' => true, 'show_snippet' => true, 'show_readmore' => true, 'post_style' => 'elegantwp-fp04-post-style-1', 'post_type' => 'recentposts', 'featured_cat' => '', 'featured_tag' => '', 'snippet_length' => 25, 'readmore_text' => esc_html__( 'Continue Reading...', 'elegantwp-pro' ) );
...
?>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'post_style' ) ); ?>"><?php esc_html_e( 'Choose the Post Style to display', 'elegantwp-pro' ); ?></label>
            <select name="<?php echo esc_attr( $this->get_field_name( 'post_style' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'post_style' ) ); ?>" class="widefat">
                <?php
                foreach ( $poststyles as $poststylevalue => $poststylename ) {
                    echo '<option value="' . esc_attr($poststylevalue) . '" id="' . esc_attr($poststylevalue) . '"', $instance['post_style'] == $poststylevalue ? ' selected="selected"' : '', '>', esc_html($poststylename), '</option>';
                }
                ?>
            </select>
        </p>
...
<?php
  ...
  function update($new_instance,$old_instance)
  {
    ...
    $instance['post_style'] = sanitize_text_field( $new_instance['post_style'] );
  ...
  function widget($args,$instance)
  {
    ...
    $post_style = ( ! empty( $instance['post_style'] ) ) ? sanitize_text_field( $instance['post_style'] ) : 'elegantwp-fp04-post-style-1';
    ...
    if ($elegantwplabelposts->have_posts()) :
        ...
        <div class="elegantwp-fp04-post <?php echo esc_attr($post_style); ?>">
            <?php if(has_post_thumbnail() && $show_thumbnail) { ?>
            <div class="elegantwp-fp04-post-thumbnail">
                <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_post_thumbnail('elegantwp-medium-image', array('class' => 'elegantwp-fp04-post-thumbnail-img', 'title' => get_the_title())); ?></a>
            </div>
            <?php } ?>
            <?php if(has_post_thumbnail() && $show_thumbnail) { ?><div class="elegantwp-fp04-post-details"><?php } ?>
            <?php if(!has_post_thumbnail() || !$show_thumbnail) { ?><div class="elegantwp-fp04-post-details-full"><?php } ?>
            <?php
            if($show_cats) {
            ...
            ?>
            <h3 class="elegantwp-fp04-post-title"><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_title(); ?></a></h3>
            <div class="elegantwp-fp04-post-footer">
            <?php if($show_author) { ?><span class="elegantwp-fp04-post-author elegantwp-fp04-post-meta"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>"><?php echo esc_html( get_the_author() ); ?></a></span><?php } ?>
            <?php if($show_date) { ?><span class="elegantwp-fp04-post-date elegantwp-fp04-post-meta"><?php echo get_the_date('d-m-Y'); ?></span><?php } ?>
            <?php if($show_comments) { ?><span class="elegantwp-fp04-post-comment elegantwp-fp04-post-meta"><?php comments_popup_link( esc_attr__( '0 Comments', 'elegantwp-pro' ), esc_attr__( '1 Comment', 'elegantwp-pro' ), esc_attr__( '% Comments', 'elegantwp-pro' ) ); ?></span><?php } ?>
            </div>
            <?php if($show_snippet) { ?><div class="elegantwp-fp04-post-snippet"><?php echo esc_html( wp_trim_words( get_the_content(), $snippet_length, '...' ) ); ?></div><?php } ?>
            <?php if($show_readmore) { ?><div class="elegantwp-fp04-post-read-more"><a href="<?php the_permalink() ?>"><?php echo esc_html( $readmore_text ); ?></a></div><?php } ?>
...
function elegantwp_posts_widget_four_function() {
    register_widget( 'elegantwp_posts_widget_four' );
}
add_action( 'widgets_init', 'elegantwp_posts_widget_four_function' );