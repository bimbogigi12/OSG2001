...
class elegantwp_posts_widget_thirteen extends WP_Widget
{

  function __construct()
  {
    $widget_ops = array( 'classname' => 'elegantwp-featured-posts-widget-thirteen', 'description' => esc_html__( 'This widget displays recent/popular/random posts or displays posts from a given category or tag.', 'elegantwp-pro' ) );
    $control_ops = array( 'id_base' => 'elegantwp-featured-posts-widget-thirteen-id' );
    parent::__construct( 'elegantwp-featured-posts-widget-thirteen-id', esc_html__( 'ElegantWP Featured Posts - Style 13', 'elegantwp-pro' ), $widget_ops, $control_ops );
  }
  ...
  function form($instance)
  {
    ...
    $defaults = array( 'title' => esc_html__( 'Featured Posts - Style 13', 'elegantwp-pro' ), 'number' => 6, 'show_thumbnail' => true, 'show_author' => false, 'show_date' => true, 'show_comments' => true, 'post_type' => 'recentposts', 'featured_cat' => '', 'featured_tag' => '' );
...
?>
        ...
        <p>
            ...
            <br /><small><?php esc_html_e('Default value: 6','elegantwp-pro'); ?></small>
        </p>
        <p>
    ...
    if ($elegantwplabelposts->have_posts()) :
        ...
        <div class="elegantwp-fp13-post">
            ...
            <div class="elegantwp-fp13-post-thumbnail">
                <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_post_thumbnail('elegantwp-medium-image', array('class' => 'elegantwp-fp13-post-thumbnail-img', 'title' => get_the_title())); ?></a>
            </div>
            <?php } ?>
            <?php if(has_post_thumbnail() && $show_thumbnail) { ?><div class="elegantwp-fp13-post-details"><?php } ?>
            <?php if(!has_post_thumbnail() || !$show_thumbnail) { ?><div class="elegantwp-fp13-post-details-full"><?php } ?>
            <h3 class="elegantwp-fp13-post-title"><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_title(); ?></a></h3>
            <?php if($show_author || $show_date || $show_comments) { ?>
            <div class="elegantwp-fp13-post-footer">
            <?php if($show_author) { ?><span class="elegantwp-fp13-post-author elegantwp-fp13-post-meta"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>"><?php echo esc_html( get_the_author() ); ?></a></span><?php } ?>
            <?php if($show_date) { ?><span class="elegantwp-fp13-post-date elegantwp-fp13-post-meta"><?php echo get_the_date('d-m-Y'); ?></span><?php } ?>
            <?php if($show_comments) { ?><span class="elegantwp-fp13-post-comment elegantwp-fp13-post-meta"><?php comments_popup_link( esc_attr__( '0 Comments', 'elegantwp-pro' ), esc_attr__( '1 Comment', 'elegantwp-pro' ), esc_attr__( '% Comments', 'elegantwp-pro' ) ); ?></span><?php } ?>
            </div>
            <?php } ?>
...
function elegantwp_posts_widget_thirteen_function() {
    register_widget( 'elegantwp_posts_widget_thirteen' );
}
add_action( 'widgets_init', 'elegantwp_posts_widget_thirteen_function' );