...
class elegantwp_posts_widget_eleven extends WP_Widget
{

  function __construct()
  {
    $widget_ops = array( 'classname' => 'elegantwp-featured-posts-widget-eleven', 'description' => esc_html__( 'This widget displays recent/popular/random posts or displays posts from a given category or tag.', 'elegantwp-pro' ) );
    $control_ops = array( 'id_base' => 'elegantwp-featured-posts-widget-eleven-id' );
    parent::__construct( 'elegantwp-featured-posts-widget-eleven-id', esc_html__( 'ElegantWP Featured Posts - Style 11', 'elegantwp-pro' ), $widget_ops, $control_ops );
  }
...
<?php
  ...
  function widget($args,$instance)
  {
    ...
    ?><div class="elegantwp-fp11-posts-wrapper"><?php

    if ($elegantwplabelpostsone->have_posts()) :

        ?><div class="elegantwp-fp11-posts-left"><?php
        if(!empty($title_one)) { echo $before_title.$title_one.$after_title; } ?>

        <div class="elegantwp-fp11-posts">
        <?php $x=0; while ($elegantwplabelpostsone->have_posts()) : $elegantwplabelpostsone->the_post();  ?>

        <?php if($x==0){ ?>

        <div class="elegantwp-fp11-post">
            <div class="elegantwp-fp11-post-thumbnail">
            <?php if(has_post_thumbnail()) { ?>
                <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_post_thumbnail('elegantwp-medium-image', array('class' => 'elegantwp-fp11-post-img', 'title' => get_the_title())); ?></a>
            <?php } else { ?>
                <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><img src="<?php echo esc_url( get_template_directory_uri() . '/assets/images/no-image-4-3.jpg' ); ?>" class="elegantwp-fp11-post-img"/></a>
            <?php } ?>
            </div>
            <div class="elegantwp-fp11-post-details">
            <?php
            if($show_cats) {
            if ( 'post' == get_post_type() ) {
                ...
                if ( $categories_list ) {
                    ...
                    printf( '<div class="elegantwp-fp11-post-categories">' . __( '<span class="screen-reader-text">Posted in </span>%1$s', 'elegantwp-pro' ) . '</div>', $categories_list ); // WPCS: XSS OK.
                }
            }
            }
            ?>
            <h3 class="elegantwp-fp11-post-title"><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_title(); ?></a></h3>
            <div class="elegantwp-fp11-post-footer">
            <?php if($show_author) { ?><span class="elegantwp-fp11-post-meta elegantwp-fp11-post-author"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>"><?php echo esc_html( get_the_author() ); ?></a></span><?php } ?>
            <?php if($show_date) { ?><span class="elegantwp-fp11-post-meta elegantwp-fp11-post-date"><?php echo get_the_date('d-m-Y'); ?></span><?php } ?>
            <?php if($show_comments) { ?><span class="elegantwp-fp11-post-meta elegantwp-fp11-post-comment"><?php comments_popup_link( esc_attr__( '0 Comments', 'elegantwp-pro' ), esc_attr__( '1 Comment', 'elegantwp-pro' ), esc_attr__( '% Comments', 'elegantwp-pro' ) ); ?></span><?php } ?>
            </div>
            <?php if($show_snippet) { ?><div class="elegantwp-fp11-post-snippet"><?php echo esc_html( wp_trim_words( get_the_content(), $snippet_length, '...' ) ); ?></div><?php } ?>
            <?php if($show_readmore) { ?><div class="elegantwp-fp11-post-read-more"><a href="<?php the_permalink() ?>"><?php echo esc_html( $readmore_text ); ?></a></div><?php } ?>
            </div>
        </div>
    ...
    if ($elegantwplabelpoststwo->have_posts()) :
        ...
        ?><div class="elegantwp-fp11-posts-right"><?php
...
function elegantwp_posts_widget_eleven_function() {
    register_widget( 'elegantwp_posts_widget_eleven' );
}
add_action( 'widgets_init', 'elegantwp_posts_widget_eleven_function' );