...
class elegantwp_posts_widget_five extends WP_Widget
{

  function __construct()
  {
    $widget_ops = array( 'classname' => 'elegantwp-featured-posts-widget-five', 'description' => esc_html__( 'This widget displays recent/popular/random posts or displays posts from a given category or tag.', 'elegantwp-pro' ) );
    $control_ops = array( 'id_base' => 'elegantwp-featured-posts-widget-five-id' );
    parent::__construct( 'elegantwp-featured-posts-widget-five-id', esc_html__( 'ElegantWP Featured Posts - Style 5', 'elegantwp-pro' ), $widget_ops, $control_ops );
  }
  ...
  function form($instance)
  {
    ...
    $defaults = array( 'title' => esc_html__( 'Featured Posts - Style 5', 'elegantwp-pro' ), 'number' => 4, 'show_thumbnail' => true, 'show_cats' => true, 'show_author' => true, 'show_date' => true, 'show_comments' => true, 'show_snippet' => true, 'show_readmore' => true, 'post_type' => 'recentposts', 'featured_cat' => '', 'featured_tag' => '', 'snippet_length' => 25, 'readmore_text' => esc_html__( 'Continue Reading...', 'elegantwp-pro' ) );
...
<?php
  ...
  function widget($args,$instance)
  {
    ...
    if ($elegantwplabelposts->have_posts()) :
        ...
        <div class="elegantwp-fp05-posts">
        <?php while ($elegantwplabelposts->have_posts()) : $elegantwplabelposts->the_post();  ?>
        <div class="elegantwp-fp05-post">
            <?php if(has_post_thumbnail() && $show_thumbnail) { ?>
            <div class="elegantwp-fp05-post-thumbnail">
                <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_post_thumbnail('elegantwp-medium-image', array('class' => 'elegantwp-fp05-post-thumbnail-img', 'title' => get_the_title())); ?></a>
            </div>
            <?php } ?>
            <?php if(has_post_thumbnail() && $show_thumbnail) { ?><div class="elegantwp-fp05-post-details"><?php } ?>
            <?php if(!has_post_thumbnail() || !$show_thumbnail) { ?><div class="elegantwp-fp05-post-details-full"><?php } ?>
            <?php
            if($show_cats) {
            ...
            ?>
            <h3 class="elegantwp-fp05-post-title"><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_title(); ?></a></h3>
            <div class="elegantwp-fp05-post-footer">
            <?php if($show_author) { ?><span class="elegantwp-fp05-post-author elegantwp-fp05-post-meta"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>"><?php echo esc_html( get_the_author() ); ?></a></span><?php } ?>
            <?php if($show_date) { ?><span class="elegantwp-fp05-post-date elegantwp-fp05-post-meta"><?php echo get_the_date('d-m-Y'); ?></span><?php } ?>
            <?php if($show_comments) { ?><span class="elegantwp-fp05-post-comment elegantwp-fp05-post-meta"><?php comments_popup_link( esc_attr__( '0 Comments', 'elegantwp-pro' ), esc_attr__( '1 Comment', 'elegantwp-pro' ), esc_attr__( '% Comments', 'elegantwp-pro' ) ); ?></span><?php } ?>
            </div>
            <?php if($show_snippet) { ?><div class="elegantwp-fp05-post-snippet"><?php echo esc_html( wp_trim_words( get_the_content(), $snippet_length, '...' ) ); ?></div><?php } ?>
            <?php if($show_readmore) { ?><div class="elegantwp-fp05-post-read-more"><a href="<?php the_permalink() ?>"><?php echo esc_html( $readmore_text ); ?></a></div><?php } ?>
...
function elegantwp_posts_widget_five_function() {
    register_widget( 'elegantwp_posts_widget_five' );
}
add_action( 'widgets_init', 'elegantwp_posts_widget_five_function' );