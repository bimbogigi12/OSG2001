...
class elegantwp_posts_widget_two extends WP_Widget
{

  function __construct()
  {
    $widget_ops = array( 'classname' => 'elegantwp-featured-posts-widget-two', 'description' => esc_html__( 'This widget displays recent/popular/random posts or displays posts from a given category or tag.', 'elegantwp-pro' ) );
    $control_ops = array( 'id_base' => 'elegantwp-featured-posts-widget-two-id' );
    parent::__construct( 'elegantwp-featured-posts-widget-two-id', esc_html__( 'ElegantWP Featured Posts - Style 2', 'elegantwp-pro' ), $widget_ops, $control_ops );
  }
  ...
  function form($instance)
  {
    ...
    $defaults = array( 'title' => esc_html__( 'Featured Posts - Style 2', 'elegantwp-pro' ), 'number' => 5,  'show_cats' => true, 'show_author' => true, 'show_date' => true, 'show_comments' => true, 'show_snippet' => true, 'show_readmore' => false, 'post_type' => 'recentposts', 'featured_cat' => '', 'featured_tag' => '', 'snippet_length' => 25, 'readmore_text' => esc_html__( 'Continue Reading...', 'elegantwp-pro' ) );
...
<?php
  ...
  function widget($args,$instance)
  {
    ...
    if ($elegantwplabelposts->have_posts()) :
        ...
        <div class="elegantwp-fp02-posts">
        ...
        if(1 == $counter) {
            $wrapper_class_start = '<div class="elegantwp-fp02-posts-left">';
            $wrapper_class_end = '</div>';
        } else {
            if(2 == $counter) {
                $wrapper_class_start = '<div class="elegantwp-fp02-posts-right">';
            }
        ...
        <div class="elegantwp-fp02-post">
            <div class="elegantwp-fp02-post-thumbnail">
            <?php if(has_post_thumbnail()) { ?>
                <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_post_thumbnail('elegantwp-medium-image', array('class' => 'elegantwp-fp02-post-img', 'title' => get_the_title())); ?></a>
            <?php } else { ?>
            ...
            ?>
            <?php } ?>
            <h3 class="elegantwp-fp02-post-title"><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'elegantwp-pro' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_title(); ?></a></h3>
            <div class="elegantwp-fp02-post-footer">
            <?php if(1 == $counter) { ?><?php if($show_author) { ?><span class="elegantwp-fp02-post-meta elegantwp-fp02-post-author"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>"><?php echo esc_html( get_the_author() ); ?></a></span><?php } ?><?php } ?>
            <?php if($show_date) { ?><span class="elegantwp-fp02-post-meta elegantwp-fp02-post-date"><?php echo get_the_date('d-m-Y'); ?></span><?php } ?>
            <?php if($show_comments) { ?><span class="elegantwp-fp02-post-meta elegantwp-fp02-post-comment"><?php comments_popup_link( esc_attr__( '0 Comments', 'elegantwp-pro' ), esc_attr__( '1 Comment', 'elegantwp-pro' ), esc_attr__( '% Comments', 'elegantwp-pro' ) ); ?></span><?php } ?>
            </div>
            <?php if(1 == $counter) { ?>
            <?php if($show_snippet) { ?><div class="elegantwp-fp02-post-snippet"><?php echo esc_html( wp_trim_words( get_the_content(), $snippet_length, '...' ) ); ?></div><?php } ?>
            <?php if($show_readmore) { ?><div class="elegantwp-fp02-post-read-more"><a href="<?php the_permalink() ?>"><?php echo esc_html( $readmore_text ); ?></a></div><?php } ?>
            <?php } ?>
            </div>
        </div>
...
function elegantwp_posts_widget_two_function() {
    register_widget( 'elegantwp_posts_widget_two' );
}
add_action( 'widgets_init', 'elegantwp_posts_widget_two_function' );